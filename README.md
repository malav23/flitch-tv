...
This repository is a playable prototype of that idea: a dark room, one screen, and a loop you can feel end to end. There is no backend yet; the audience, the odds, and the video generation are simulated.
...
- **One typeface.** Space Mono, uppercase, three opacities of white. No serif, no colour except a hot red for danger and RGB cyan/magenta/yellow slices that flash across the frame on every transition.
